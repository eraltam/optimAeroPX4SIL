function runMissionSILF22(sessionId, opts)
%RUNMISSIONSILF22 Set up and start an f22JSBSim SIL run against an already-running PX4 + harness.
%
% Clone of runMissionSILC130.m, pointed at vehicleType="f22JSBSim" and its own build cache -- see
% PLAN_JSBSIM_SFUNCTION_F22_C130.md Phase F.
%
% Run HILDiagnostics\start_mission_f22.ps1 first (WSL relay + PX4 + Windows harness), wait for
% it to report PX4 is up, then call this with the SAME session id.
%
% runMissionSILF22(sessionId) starts a run with PassThrough (no external) visualization.
%
% Name-value options:
...
%
% Examples:
%   runMissionSILF22("run1")
%   runMissionSILF22("run2", "visualizationType", "FlightGear")
%
...
% start_mission_f22.ps1 and this function with a new session id.
...
initVehicleSIL("launchFullSIL", false, "vehicleType", "f22JSBSim", ...
...
cleanBuildRoot = fullfile(diagnosticsRoot, "simulink_build_f22JSBSim");
