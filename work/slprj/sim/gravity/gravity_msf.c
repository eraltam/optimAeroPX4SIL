...
#define S_FUNCTION_NAME gravity_msf
#endif
...
rt_GetOSigstreamManager ( void ) { return NULL ; } void * rt_slioCatalogue ( void ) { return NULL ; } void * rtwGetPointerFromUniquePtr ( void * uniquePtr ) { return NULL ; } void * CreateDiagnosticAsVoidPtr ( const char * id , int nargs , ... ) { void * voidPtrDiagnostic = NULL ; va_list args ; va_start ( args , nargs ) ; slmrCreateDiagnostic ( id , nargs , args , & voidPtrDiagnostic ) ; va_end ( args ) ; return voidPtrDiagnostic ; } void rt_ssSet_slErrMsg ( void * S , void * diag ) { SimStruct * simStrcut = ( SimStruct * ) S ; if ( ! _ssIsErrorStatusAslErrMsg ( simStrcut ) ) { _ssSet_slLocalErrMsg ( simStrcut , diag ) ; } else { _ssDiscardDiagnostic ( simStrcut , diag ) ; } } void rt_ssReportDiagnosticAsWarning ( void * S , void * diag ) { _ssReportDiagnosticAsWarning ( ( SimStruct * ) S , diag ) ; } void rt_ssReportDiagnosticAsInfo ( void * S , void * diag ) { _ssReportDiagnosticAsInfo ( ( SimStruct * ) S , diag ) ; } const char * rt_CreateFullPathToTop ( const char * toppath , const char * subpath ) { char * fullpath = NULL ; slmrCreateFullPathToTop ( toppath , subpath , & fullpath ) ; return fullpath ; } boolean_T slIsRapidAcceleratorSimulating ( void ) { return false ; } void rt_RAccelReplaceFromFilename ( SimStruct * S , const char * blockpath , char * fileName ) { ( void ) blockpath ; ( void ) fileName ; } void rt_RAccelReplaceToFilename ( SimStruct * S , const char * blockpath , char * fileName ) { ( void ) blockpath ; ( void ) fileName ; } void * slsa_malloc ( size_t s ) { return malloc ( s ) ; } void slsa_free ( void * ptr ) { free ( ptr ) ; } static void mdlPeriodicOutputUpdate ( SimStruct * S , int_T tid ) { e53sj53lngm * dw = ( e53sj53lngm * ) ssGetDWork ( S , 0 ) ; real_T const * i_p2clcbu0ic = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ; real_T const * i_lfa41gdewj = ( real_T * ) ssGetInputPortSignal ( S , 10 ) ; real_T const * i_jyw1wv1hnz = ( real_T * ) ssGetInputPortSignal ( S , 12 ) ; real_T * o_B_1_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; if ( tid == 0 ) { gravity ( & ( dw -> rtm ) , i_p2clcbu0ic , i_lfa41gdewj , i_jyw1wv1hnz , o_B_1_1 , & ( dw -> rtdw ) ) ; } } static void mdlInitializeSizes ( SimStruct * S ) { if ( ( S -> mdlInfo -> genericFcn != ( NULL ) ) && ( ! ( S -> mdlInfo -> genericFcn ) ( S , GEN_FCN_CHK_MODELREF_SFUN_HAS_MODEL_BLOCK , - 1 , ( NULL ) ) ) ) { return ; } ssSetNumSFcnParams ( S , 0 ) ; ssFxpSetU32BitRegionCompliant ( S , 1 ) ; if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S -> mdlInfo -> genericFcn ; } ssSetRTWGeneratedSFcn ( S , 2 ) ; ssSetNumContStates ( S , 0 ) ; ssSetNumDiscStates ( S , 0 ) ; ssSetSymbolicDimsSupport ( S , true ) ; slmrInitializeIOPortDataVectors ( S , 16 , 2 ) ; if ( ! ssSetNumInputPorts ( S , 16 ) ) return ; if ( ! ssSetInputPortVectorDimension ( S , 0 , 1 ) ) return ; ssSetInputPortDimensionsMode ( S , 0 , FIXED_DIMS_MODE ) ; ssSetInputPortFrameData ( S , 0 , FRAME_NO ) ; if ( ssGetSimMode ( S ) != SS_SIMMODE_SIZES_CALL_ONLY ) { ssSetInputPortDataType ( S , 0 , SS_DOUBLE ) ; } if ( ssGetSimMode ( S ) != SS_SIMMODE_SIZES_CALL_ONLY ) {
...
ssSetInputPortMatrixDimensions ( S , 10 , 3 , 3 ) ) return ;
...
ssSetInputPortMatrixDimensions ( S , 14 , 3 , 3 ) ) return ;
...
mr_gravity_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S ,
...
SS_SIMMODE_SIZES_CALL_ONLY ) { int_T retVal = 1 ; mr_gravity_MdlInfoRegFcn ( S
, "gravity" , & retVal ) ; if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
...
( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( e53sj53lngm ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { e53sj53lngm * dw = ( e53sj53lngm * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int_T contextTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid ( S , & contextTid ) ; if ( contextTid == CONSTANT_TID ) { contextTid = 0 ; } simTgtAllocOPModelData ( S ) ; hjpfxdeo4m ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) , slmrGetTopTidFromMdlRefChildTid ( S , 1 , true ) , & ( dw -> rtm ) , & ( dw -> rtdw ) , sysRanPtr , contextTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ; ssSetModelMappingInfoPtr ( S , & ( dw -> rtm . DataMapInfo . mmi ) ) ; if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S -> mdlInfo -> genericFcn ; } b4jvsait25 ( & ( dw -> rtm ) , & ( dw -> rtdw ) ) ; }
#define MDL_START
static void mdlStart ( SimStruct * S ) { } static void mdlOutputs ( SimStruct
* S , int_T tid ) { e53sj53lngm * dw = ( e53sj53lngm * ) ssGetDWork ( S , 0 )
; real_T const * i_p2clcbu0ic = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ;
real_T const * i_lfa41gdewj = ( real_T * ) ssGetInputPortSignal ( S , 10 ) ;
real_T const * i_jyw1wv1hnz = ( real_T * ) ssGetInputPortSignal ( S , 12 ) ;
real_T * o_B_1_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T *
o_B_1_2 = ( real_T * ) ssGetOutputPortSignal ( S , 1 ) ; if ( tid ==
PARAMETER_TUNING_TID ) { gravityTID1 ( o_B_1_2 ) ; } if ( tid != CONSTANT_TID
&& tid != PARAMETER_TUNING_TID ) { gravity ( & ( dw -> rtm ) , i_p2clcbu0ic ,
i_lfa41gdewj , i_jyw1wv1hnz , o_B_1_1 , & ( dw -> rtdw ) ) ; } } static void
mdlTerminate ( SimStruct * S ) { e53sj53lngm * dw = ( e53sj53lngm * )
ssGetDWork ( S , 0 ) ; ebphgotimp ( & ( dw -> rtm ) ) ; simTgtFreeOPModelData
( S ) ; return ; }
#define MDL_CLEANUP_RUNTIME_RESOURCES
static void mdlCleanupRuntimeResources ( SimStruct * S ) { e53sj53lngm * dw =
( e53sj53lngm * ) ssGetDWork ( S , 0 ) ; ei5jam3ejs ( & ( dw -> rtm ) , & ( dw
-> rtdw ) ) ; }
...
mxArray * mdlrefDW = mr_gravity_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_gravity_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss , 0 , 2
...
simTgtDeserializeVoidMemoryOP ( S , voidMemData ) ; } mr_gravity_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss , 0 , 1 ) ) ; ssSetTNext ( S , ( time_T ) mxGetScalar ( mxGetFieldByNumber ( ss , 0 , 3 ) ) ) ; ssSetTNextTid ( S , ( int_T ) mxGetScalar ( mxGetFieldByNumber ( ss , 0 , 4 ) ) ) ; }
