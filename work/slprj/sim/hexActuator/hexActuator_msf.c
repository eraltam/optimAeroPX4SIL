...
mxCreateStructMatrix ( 1 , 1 , 6 , simStateFieldNames ) ; { const c1n3c5jiq0
* localX = ( const c1n3c5jiq0 * ) ssGetContStates ( S ) ; const size_t
numBytes = sizeof ( c1n3c5jiq0 ) ; mxArray * storedX = mxCreateNumericMatrix
...
mr_hexActuator_GetDWork ( ssGetDWork ( S , 0 ) ) ; mxSetFieldByNumber ( ss ,
...
c1n3c5jiq0 * localX = ( c1n3c5jiq0 * ) ssGetContStates ( S ) ; const size_t
numBytes = sizeof ( c1n3c5jiq0 ) ; const mxArray * storedX =
mxGetFieldByNumber ( ss , 0 , 0 ) ; const UINT8_T * rawData = ( const UINT8_T
* ) mxGetData ( storedX ) ; memcpy ( localX , & rawData [ 0 ] , numBytes ) ;
} mr_hexActuator_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss ,
