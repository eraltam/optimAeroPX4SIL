function [accel_vib_mps2, gyro_vib_radps] = DIAG_C8R_no_accel_0to5hz_ANELLO_X3_Vibration_LevelA_fcn(rotorAngVel_radps, enableVibration, seed_u32)
% DIAG_C8R_NO_ACCEL_0TO5HZ -- DIAGNOSTIC ONLY, not a candidate. Isolation test per
% M2_C8R_C9R_HIL_FAIL_ASSESSMENT.md option 1: identical to C8R in every respect except
% G_accel(1) (the accel 0-5Hz broadband gain, C8R's single largest gain, 2.2177146100) is zeroed.
%
% Purpose: confirm, offline, that this specific term is really what's responsible for closing the
% accel 0-5Hz D3 gap (i.e. that removing it reopens the gap back toward production's baseline) --
% establishing the trade-off directly rather than assuming it. This does NOT test the EKF-arming
% mechanism itself (no offline EKF2 replay capability exists in this project yet -- confirmed
% separately, would need new glue code around px4_benchmark.py / a ULog accel-injection step, not
% built here) -- only the offline spectral side of the trade-off.
%
% Never wire this into any .slx, never fly it, never promote it. Delete or ignore once the
% isolation question is answered.
...
% ---- ONLY CHANGE FROM C8R: G_accel(1) zeroed (was 2.2177146100) -----------------------------
G_accel = [0.0000000000, 0.5014011500, 1.1786216400, 0.7244736800];   % [0-5, 5-15, 18-25, 30-45] Hz
