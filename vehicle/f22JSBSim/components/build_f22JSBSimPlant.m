%% build_f22JSBSimPlant.m
% Populates f22JSBSim.slx's "physics" middle section, same pattern as
% vehicle/c130JSBSim/components/build_c130JSBSimPlant.m -- see PLAN_JSBSIM_SFUNCTION_F22_C130.md
% Phase C. The actuators Model Reference has already been repointed from actuatorsC172p.slx to
% this vehicle's own local actuatorsF22JSBSim.slx (a byte-for-byte clone, see setUpActuators.m's
% "f22jsbsim" case for why its inert enginePistonPropC172p.slx Model Reference stays in place),
...
%
% Differences from build_c172pJSBSimPlant.m/build_c130JSBSimPlant.m, all driven by F-22's own
% io-config (f22JSBSim_io.xml):
%   - packF22JSBSimInputs.m outputs an 8-element vector (aileron, elevator, rudder, throttle
%     [bare, engine 0], throttle[1] [engine 1], dynamic gear, left brake, right brake) -- still only ONE MATLAB Function
%     block, still only one signal into JSBSim_SFunction's input port. Note the pack function's own
%     MATLAB signature still only takes 4 scalar args (posAileron_rad, posElevator_rad,
%     posRudder_rad, cmdThrottle_unit) -- the broadcast to both throttle indices and the
%     gear-cmd-norm constant happen INSIDE packF22JSBSimInputs.m, not via extra Simulink inputs.
%   - JSBSim_SFunction's block parameters point at aircraft='f22' (matching
%     aircraft/f22/f22.xml), reset_file='f22JSBSimInit', and this repo's own f22JSBSim_io.xml.
%   - unpackF22JSBSimOutputs.m has only 5 inputs (jsbsimState, jsbsimAero, gravityScalar_mps2,
%     referenceLatitude_deg, referenceLongitude_deg) -- same shape as c130JSBSim's own unpack
%     function, confirmed aircraft-independent.
%
...
%   BusDefinition('f22JSBSim'); stepSize_s = 0.004;
% (f22JSBSim shares c172p's/c172pJSBSim's/c130JSBSim's Servos/ServosCommand/Failure bus
% definitions -- see Phase D.)
%
% PREREQUISITE: f22JSBSim.slx must already exist (cloned from vehicle/c172pJSBSim/c172pJSBSim.slx)
% with the actuators Model Reference repointed to actuatorsF22JSBSim.slx and the same 18 blocks
...
mdl = 'f22JSBSim';
if ~bdIsLoaded(mdl)
    error('build_f22JSBSimPlant:NotLoaded', ...
...
thisDir = fileparts(mfilename('fullpath'));   % vehicle/f22JSBSim/components
...
%% ====================================================================
%% 2. packF22JSBSimInputs (MATLAB Function)
%% ====================================================================
packBlk = [mdl '/packF22JSBSimInputs'];
...
for i = 1:numel(charts)
    if isempty(packChart) && contains(charts(i).Path, 'packF22JSBSimInputs')
        packChart = charts(i);
    end
end
if isempty(packChart)
    error('build_f22JSBSimPlant:ChartNotFound', 'Could not find packF22JSBSimInputs chart.');
end
packChart.Script = fileread(fullfile(thisDir, 'packF22JSBSimInputs.m'));

add_line(mdl, 'Select_ServoPos/1', 'packF22JSBSimInputs/1', 'autorouting', 'on');
add_line(mdl, 'Select_ServoPos/2', 'packF22JSBSimInputs/2', 'autorouting', 'on');
add_line(mdl, 'Select_ServoPos/3', 'packF22JSBSimInputs/3', 'autorouting', 'on');
add_line(mdl, 'Select_Throttle/1', 'packF22JSBSimInputs/4', 'autorouting', 'on');

%% ====================================================================
%% 3. JSBSim_SFunction block
%% ====================================================================
sfunBlk = [mdl '/JSBSim_SFunction'];
ioConfigPath = strrep(fullfile(silRoot, 'f22JSBSim_io.xml'), '\', '/');
% io_config uses an ABSOLUTE path deliberately -- same CWD-at-compile-time reasoning as
% c172pJSBSim's/c130JSBSim's own JSBSim_SFunction block (a relative 'f22JSBSim_io' resolves
% against MATLAB's CWD at compile time, which differs between a standalone update_diagram and
% compiling nested under the full VehicleSilSimulation.slx). aircraft name / reset file stay
% relative -- those resolve via JSBSim's own internal SetAircraftPath("aircraft")/
% JSBSIM_ROOT_OVERRIDE mechanism.
add_block('simulink/User-Defined Functions/S-Function', sfunBlk, 'Position', [480, 560, 600, 620], ...
    'FunctionName', 'JSBSim_SFunction', ...
    'Parameters', sprintf('''f22'', stepSize_s, 0, '''', ''f22JSBSimInit'', ''%s''', ioConfigPath));
add_line(mdl, 'packF22JSBSimInputs/1', 'JSBSim_SFunction/1', 'autorouting', 'on');

%% ====================================================================
%% 4. unpackF22JSBSimOutputs (MATLAB Function)
%% ====================================================================
unpackBlk = [mdl '/unpackF22JSBSimOutputs'];
...
unpackChart = [];
for i = 1:numel(charts)
    if contains(charts(i).Path, 'unpackF22JSBSimOutputs')
        unpackChart = charts(i);
    end
end
if isempty(unpackChart)
    error('build_f22JSBSimPlant:ChartNotFound', 'Could not find unpackF22JSBSimOutputs chart.');
end
unpackChart.Script = fileread(fullfile(thisDir, 'unpackF22JSBSimOutputs.m'));

% unpackF22JSBSimOutputs inputs: jsbsimState, jsbsimAero, gravityScalar_mps2,
% referenceLatitude_deg, referenceLongitude_deg -- only 5, same shape as c130JSBSim's own.
add_line(mdl, 'JSBSim_SFunction/1', 'unpackF22JSBSimOutputs/1', 'autorouting', 'on');
add_line(mdl, 'JSBSim_SFunction/2', 'unpackF22JSBSimOutputs/2', 'autorouting', 'on');
add_line(mdl, 'Select_Gravity/1', 'unpackF22JSBSimOutputs/3', 'autorouting', 'on');
...
add_line(mdl, 'refLat_deg/1', 'unpackF22JSBSimOutputs/4', 'autorouting', 'on');
add_line(mdl, 'refLon_deg/1', 'unpackF22JSBSimOutputs/5', 'autorouting', 'on');
...
%% ====================================================================
% unpackF22JSBSimOutputs outputs (order): aircraftVelInNED_mps(1), aircraftPosInNED_m(2),
...
for i = 1:9
    add_line(mdl, sprintf('unpackF22JSBSimOutputs/%d', i), sprintf('BodyStateBus_Creator/%d', i), 'autorouting', 'on');
end
...
add_line(mdl, 'unpackF22JSBSimOutputs/10', 'AirDataBus_Creator/1', 'autorouting', 'on');
add_line(mdl, 'unpackF22JSBSimOutputs/11', 'AirDataBus_Creator/2', 'autorouting', 'on');
add_line(mdl, 'unpackF22JSBSimOutputs/12', 'AirDataBus_Creator/3', 'autorouting', 'on');
...
add_line(mdl, 'unpackF22JSBSimOutputs/13', 'VehicleConfigurationBus_Creator/1', 'autorouting', 'on');
